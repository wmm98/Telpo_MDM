...
class BasePage:
    ...
    def comm_alert_fade(self, loc):
        try:
            self.web_driver_wait_until_not(Page.EC.presence_of_element_located(loc), 5)
            return True
    ...
    def comm_alert_show(self, loc):
        try:
            self.web_driver_wait_until(Page.EC.presence_of_element_located(loc), 5)
            return True
        except Page.TimeoutException:
            return False

    def comm_confirm_alert_not_existed(self, alert_loc, ele_loc, ex_js=0):
        now_time = Page.time.time()
        while True:
            if self.comm_alert_fade(alert_loc):
                ...
                if ex_js == 1:
                    ...
                    self.click(ele_loc)
            if Page.time.time() > self.return_end_time(now_time):
    ...
    def move_and_click(self, ele):
        Page.ActionChains(self.driver).move_to_element(ele).click().perform()
    ...
    def exc_js_click_loc(self, loc):
        self.web_driver_wait_until(Page.EC.presence_of_element_located(loc))
        ele = self.get_element(loc)
        self.driver.execute_script("arguments[0].click();", ele)
    ...
    def get_elements(self, loc):
        self.web_driver_wait_until(Page.EC.presence_of_all_elements_located(loc))
    ...
    def web_driver_wait_until(self, condition, wait_times=0):
        if wait_times == 0:
            return Page.WebDriverWait(self.driver, self.times).until(condition)
        else:
            return Page.WebDriverWait(self.driver, wait_times).until(condition)

    def web_driver_wait_until_not(self, condition, wait_times=0):
        if wait_times == 0:
            return Page.WebDriverWait(self.driver, self.times).until_not(condition)
        else:
            return Page.WebDriverWait(self.driver, wait_times).until_not(condition)

    def check_ele_is_selected(self, ele):
        if not self.ele_is_selected(ele):
            self.web_driver_wait_until(Page.EC.element_to_be_selected(ele))

    def check_ele_is_not_selected(self, ele):
        if self.ele_is_selected(ele):
            self.web_driver_wait_until_not(Page.EC.element_to_be_selected(ele))
    ...
    def extract_integers(self, text):
        pattern = r"\d+"
        integers = Page.re.findall(pattern, text)
        if len(integers) != 0:
    ...
    def compare_time(self, time1, time2):
        dt1 = Page.datetime.strptime(time1, "%Y-%m-%d %H:%M")
        dt2 = Page.datetime.strptime(time2, "%Y-%m-%d %H:%M")
        if dt1 <= dt2:
            return True
        else:
            return False
